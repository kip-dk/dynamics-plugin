    ...
    public class AccountPostUpdateTest
    {
        ...
        public void ExecuteHitTest()
        {
            using (var ctx = Kipon.Xrm.Fake.Repository.PluginExecutionFakeContext.ForType<Kipon.Solid.Plugin.Plugins.Account.AccoutPostUpdate>())
            {
                ...
                Kipon.Solid.Plugin.Service.AccountService.POST_MERGE_TEST = null;
                string expect = Kipon.Solid.Plugin.Service.AccountService.PostMergedTest(account.Name, new Microsoft.Xrm.Sdk.OptionSetValue(2));
        ...
        public void ExecuteDoNotHitTest()
        {
            using (var ctx = Kipon.Xrm.Fake.Repository.PluginExecutionFakeContext.ForType<Kipon.Solid.Plugin.Plugins.Account.AccoutPostUpdate>())
            {
                ...
                var target = new Entities.Account { AccountId = account.AccountId.Value, Address1_City = "København" };
                ctx.Update(target);
            }
        }
