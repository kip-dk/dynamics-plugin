namespace Kipon.Xrm.Reflection
{
    ...
    public class ParameterType
    {
        public class Cache
        {
            private System.Reflection.ParameterInfo UOW;
            private System.Reflection.ParameterInfo UOW_ADMIN;
            public Types Types { get; private set; }

            private PluginMethod.Cache pluginCache;
            public Cache(PluginMethod.Cache pluginCache)
            {
                this.pluginCache = pluginCache;
                this.Types = Types.Instance;
                var method = typeof(ParameterType).GetMethod(nameof(ParameterType.Cache.DummyUOW), System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            ...
            private Dictionary<System.Reflection.ParameterInfo, ParameterType> resolvedTypes = new Dictionary<System.Reflection.ParameterInfo, ParameterType>();

            public ParameterType ForParameter(System.Reflection.ParameterInfo parameter)
            {
                ...
                {
                    resolvedTypes[parameter] = new ParameterType(this) { FromType = type, ToType = type };
                ...
                if (!type.IsInterface && !type.IsAbstract)
                {
                    ...
                    {
                        var result = new ParameterType(this) { FromType = type, ToType = type };
                        var isEntity = false;
                ...
                if (type.IsInterface)
                {
                    ...
                    if (toType != null)
                    {
                        var result = new ParameterType(this) { FromType = type, ToType = toType, IsTarget = true };
                        var entity = (Microsoft.Xrm.Sdk.Entity)Activator.CreateInstance(result.ToType);
                        result.LogicalName = entity.LogicalName;
                        result.ResolveProperties();
                    ...
                    if (toType != null)
                    {
                        var result = new ParameterType(this) { FromType = type, ToType = toType, IsPreimage = true };
                        var entity = (Microsoft.Xrm.Sdk.Entity)Activator.CreateInstance(result.ToType);
                        result.LogicalName = entity.LogicalName;
                        result.ResolveProperties();
                        if (ReturnIfOk(type, result))
                    ...
                    if (toType != null)
                    {
                        var result = new ParameterType(this) { FromType = type, ToType = toType, IsMergedimage = true };
                        var entity = (Microsoft.Xrm.Sdk.Entity)Activator.CreateInstance(result.ToType);
                        result.LogicalName = entity.LogicalName;
                        result.ResolveProperties();
                        if (ReturnIfOk(type, result))
                    ...
                    if (toType != null)
                    {
                        var result = new ParameterType(this) { FromType = type, ToType = toType, IsPostimage = true };
                        var entity = (Microsoft.Xrm.Sdk.Entity)Activator.CreateInstance(result.ToType);
                        result.LogicalName = entity.LogicalName;
                        result.ResolveProperties();
                        if (ReturnIfOk(type, result))
                ...
                if (type.IsInterface)
                {
                    ...
                    var result = new ParameterType(this) { FromType = type, ToType = r1, Constructor = GetConstructor(r1) };
            ...
            public ParameterType ForQuery(Type type)
            {
                ...
                if (type == queryType)
                {
                    var result = new ParameterType(this) { FromType = type, ToType = queryType, IsQuery = true };
                    return result;
                }
                return null;
            }

            public ParameterType ForUow(bool admin)
            {
                ...
                var result = new ParameterType(this) { FromType = fromType, ToType = r1, Constructor = GetConstructor(r1), RequireAdminService = admin };
                resolvedTypes[pi] = result;
                return result;
            }

            #region private static helpers
            private Type GetInterfaceImplementation(Type type)
            {
            ...
            private System.Reflection.ConstructorInfo GetConstructor(Type type)
            {
                var constructors = type.GetConstructors(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
            ...
            private bool ReturnIfOk(Type from, ParameterType result)
            {
        ...
        #region constructor
        private ParameterType(Cache cache)
        {
            this.cache = cache;
        }
        #endregion
        ...
        public System.Reflection.PropertyInfo RepositoryProperty
        {
            get
            {
                ...
                var uowTC = this.cache.ForUow(this.RequireAdminService);
        ...
        public string ObjectInstanceKey
        {
            get
            {
                if (_ik == null)
                {
                    ...
                    else if (this.FromType.Implements(this.cache.Types.IAdminUnitOfWork))
                    {
                        this._ik = this.cache.Types.IAdminUnitOfWork.FullName;
                    ...
                    else if (this.FromType.Implements(this.cache.Types.IAdminUnitOfWork)) this._ik = this.cache.Types.IAdminUnitOfWork.FullName;
                    else if (this.FromType.Implements(this.cache.Types.IUnitOfWork)) this._ik = this.cache.Types.IUnitOfWork.FullName;
